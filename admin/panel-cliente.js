...
async function cancelarCita(id) {
  if (!id) {
    // Si no se pasa ID, intentar buscar la cita activa actual del appState
    const sb = await getSupabase();
    const { data: cita } = await sb.from('citas')
      .select('id')
  ...
  confirmarAccion('Cancelar Cita', '¿Estás seguro de que deseas cancelar tu cita? Esta acción no se puede deshacer.', async () => {
    try {
      const sb = await getSupabase();
...
window.copiarLinkReferido = copiarLinkReferido;
window.compartirReferido = compartirReferido;
window.cargarSlotsInteligente = cargarSlotsInteligente;
window.confirmarReservaManual = confirmarReservaManual;
